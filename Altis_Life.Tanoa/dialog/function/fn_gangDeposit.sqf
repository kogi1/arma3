...
if ((group player getVariable ["gbank_in_use_by",player]) != player) exitWith {_huan = localize "STR_ATM_WithdrawMin"; [_huan,"red","slow"] call life_fnc_notify;}; //Check if it's in use.
...
_huan = format [localize "STR_ATM_DepositSuccessG",[_value] call life_fnc_numberText];
