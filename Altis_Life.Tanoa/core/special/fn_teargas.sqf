...
[] spawn {
	while {true} do 
	{
		...
		if (goggles player != "G_Balaclava_blk") then {antidote1 = 2583}; // Player has no Suit
		if (goggles player == "G_Balaclava_blk") then {antidote1 = 2581}; // Player carries the Gas Suit
