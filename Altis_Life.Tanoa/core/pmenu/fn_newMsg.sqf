...
switch(_type) do{ 
 ...
 //normal message 
 case 1: { 
 if(isNUll life_smartphoneTarget) exitWith {_huan = format["Keine Person ausgewählt!"];
...
[life_smartphoneTarget,_msg,player,0] remoteExec ["TON_fnc_handleMessages",2];
 ...
 //copmessage 
 case 2: { if(({side _x == west} count playableUnits) == 0) exitWith {_huan = format["Die Polizei ist momentan nicht erreichbar, bitte versuchen Sie es später erneut."];
 [_huan,"red","slow"] call life_fnc_notify;
};
...
[ObjNull,_msg,player,5] remoteExec ["TON_fnc_handleMessages",2];
 ...
 closeDialog 887890;
  ...
  case 8: { if((FETCH_CONST(life_coplevel) < 3)) exitWith {_huan = "Du bist dazu nicht berächtigt!";
  [_huan,"red","slow"] call life_fnc_notify;
};
 if(_msg == "") exitWith {_huan = "Du kannst keine leere Nachricht senden!";
 [_huan,"red","slow"] call life_fnc_notify;
