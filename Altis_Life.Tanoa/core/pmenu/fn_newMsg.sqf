...
[life_smartphoneTarget,_msg,player,0] remoteExec ["TON_fnc_handleMessages",2];
 ...
 //copmessage 
 case 2: { if(({side _x == west} count playableUnits) == 0) exitWith {hint format["Die Polizei ist momentan nicht erreichbar, bitte versuchen Sie es später erneut."];
};
...
[ObjNull,_msg,player,3] remoteExec ["TON_fnc_handleMessages",2];
 hint format["Du hast eine Nachricht an die Rettungs/ADAC Dienststelle gesendet.",_msg];
 ctrlShow[888899,true];
 closeDialog 887890;
 };
 //adminToPerson 
...
[life_smartphoneTarget,_msg,player,4] remoteExec ["TON_fnc_handleMessages",2];
 hint format["Admin Nachricht: %1 - Nachricht: %2",name life_smartphoneTarget,_msg];
 closeDialog 88883;
 };
 //emergencyloading 
