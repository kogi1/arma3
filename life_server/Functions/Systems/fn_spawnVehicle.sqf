...
_query = format ["SELECT id, side, classname, type, pid, alive, active, plate, color, inventory, gear, fuel, damage, blacklist, impound, insure FROM vehicles WHERE id='%1' AND pid='%2'",_vid,_pid];
...
[1,"Ihr Fahrzeug wurde bereitgestellt und ist versichert!"] remoteExecCall ["life_fnc_broadcast",_unit];
}else{
